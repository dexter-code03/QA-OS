...
export function App() {
  ...
  useEffect(() => { if (project) { refresh(); api.listDevices().then(setDevices).catch(() => {}); } }, [project, refresh]);
    ...
    <>
      ...
      <div className="layout">
        ...
        <div className="sidebar">
          ...
          <div className="sidebar-section">
            ...
            {devices.android.length === 0 && devices.ios_simulators.length === 0 && (
              <div style={{ padding: "4px 16px", fontSize: 11, color: "var(--muted)" }}>No devices detected</div>
            )}
          </div>
                  ...
                  <div key={m.id}>
                    <div className="sidebar-item" style={{ fontWeight: 600, fontSize: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <span style={{ display: "flex", gap: 2 }}>
                        <button style={{ background: "none", border: "none", color: "var(--muted)", fontSize: 9, cursor: "pointer" }} title="Rename module" onClick={async () => { const n = prompt("Rename module:", m.name); if (n && n.trim()) { try { await api.renameModule(m.id, n.trim()); refresh(); toast("Renamed", "success"); } catch (e: any) { toast(e.message, "error"); } } }}>✏️</button>
                        <button style={{ background: "none", border: "none", color: "var(--muted)", fontSize: 9, cursor: "pointer" }} title="Delete module" onClick={async () => { if (confirm(`Delete module "${m.name}" and all its suites?`)) { try { await api.deleteModule(m.id); refresh(); toast("Deleted", "info"); } catch (e: any) { toast(e.message, "error"); } } }}>🗑</button>
                      </span>
                    </div>
                    {mSuites.map(s => {
                      ...
                      return (
                        <div key={s.id} style={{ paddingLeft: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          <span style={{ display: "flex", gap: 2 }}>
                            <button style={{ background: "none", border: "none", color: "var(--muted)", fontSize: 9, cursor: "pointer" }} title="Rename suite" onClick={async () => { const n = prompt("Rename suite:", s.name); if (n && n.trim()) { try { await api.renameSuite(s.id, n.trim()); refresh(); toast("Renamed", "success"); } catch (e: any) { toast(e.message, "error"); } } }}>✏️</button>
        ...
        <div className="main-content">
          ...
          {page === "execution" && <ExecutionView project={project} tests={tests} builds={builds} runs={runs} devices={devices} activeRunId={activeRunId} onRunCreated={id => { setActiveRunId(id); refresh(); }} onRefresh={refresh} />}
  ...
  return (
    <>
      ...
      <div className="runs-grid">
        ...
        <div className="panel">
          <div className="panel-header"><div className="panel-title">Pass rate by module</div></div>
          <div className="chart-bar-group">
            ...
            {moduleStats.length === 0 && <div style={{ color: "var(--muted)", fontSize: 12 }}>Create tests to see module breakdown</div>}
          </div>
        </div>
      </div>
    </>
...
/* ── Execution ─────────────────────────────────────── */
function ExecutionView({ project, tests, builds, runs, devices, activeRunId, onRunCreated, onRefresh }: { project: Project; tests: TestDef[]; builds: Build[]; runs: Run[]; devices: DeviceList; activeRunId: number | null; onRunCreated: (id: number) => void; onRefresh: () => void }) {
  ...
  const startRun = async () => {
    ...
    setBusy(true);
    try { const r = await api.createRun({ project_id: project.id, build_id: buildId, test_id: testId, platform, device_target: deviceTarget }); setStepResults([]); setSelShot(null); onRunCreated(r.id); toast(`Run #${r.id} started`, "success"); }
    catch (e: any) { toast(e.message, "error"); }
    finally { setBusy(false); }
  };
  ...
  const stepDefs = testForRun?.steps || [];
  ...
  const loadXmlForStep = async (idx: number) => {
    const ps = pageSources[idx] || stepResults[idx]?.pageSource;
    if (ps) {
      try { const r = await fetch(artBase + ps); if (r.ok) setLiveXml(await r.text()); } catch {}
    }
  };
  ...
  const aiFixRun = async () => {
    ...
    if (failPs) {
      try { const r = await fetch(artBase + failPs); if (r.ok) failXml = await r.text(); } catch {}
    }
    if (!failXml && liveXml) failXml = liveXml;
  ...
  const applyFix = async (mode: "update" | "new") => {
    ...
    try {
      if (mode === "update") {
        await api.deleteTest(testForRun.id);
        await api.createTest(project.id, { name: testForRun.name, steps: fixResult.fixed_steps });
        toast("Test updated with fixed steps", "success");
      } else {
        await api.createTest(project.id, { name: `${testForRun.name} (fixed)`, steps: fixResult.fixed_steps });
  ...
  const rerunWithFix = async () => {
    if (!fixResult || !run) return;
    await applyFix("update");
    setTimeout(async () => {
      try {
        const updatedTests = await api.listTests(project.id);
        const updatedTest = updatedTests.find(t => t.name === testForRun?.name);
        if (updatedTest) {
          const r = await api.createRun({ project_id: run.project_id, build_id: run.build_id ?? undefined, test_id: updatedTest.id, platform: run.platform, device_target: run.device_target });
...
<div class="footer">QA·OS Automated Bug Report · ${project.name} · Run #${run.id} · ${new Date().toISOString()}</div>
    ...
    a.download = `bug_report_run_${run.id}_${testForRun.name.replace(/\s+/g, "_")}.html`;
    a.click();
    ...
    <>
      <div className="section-head" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {run && run.status === "running" && <button className="btn-ghost" style={{ fontSize: 11, padding: "7px 14px" }}>⏸ Pause</button>}
          {run && run.status === "running" && <button className="btn-ghost" style={{ fontSize: 11, padding: "7px 14px", color: "var(--danger)", borderColor: "rgba(255,59,92,.3)" }}>⏹ Stop</button>}
      ...
      {!run && (
        <div className="panel" style={{ padding: 14, marginBottom: 16 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button className="run-now-btn" onClick={startRun} disabled={busy || !testId}>▶ Start Run</button>
          </div>
        </div>
      )}
          ...
          <div className="exec-left">
            ...
            <div className="device-frame">
              ...
              <div className="device-screen">
                {selShot ? <img src={artBase + selShot} alt="Device" className="device-screenshot" /> : <div className="device-placeholder">{run.status === "running" ? "Waiting for screenshot..." : "No screenshot"}</div>}
              </div>
            </div>
            ...
            <div className="panel">
              <div className="panel-header">
                <div className="panel-title">Live XML — Current Screen</div>
                <div style={{ fontSize: 11, color: "var(--muted)" }}>Captured before each step</div>
              </div>
              <div style={{ padding: 12 }}>
                <div className="xml-panel">
                  {liveXml ? liveXml.slice(0, 2000) : (run.status === "running" ? "Waiting for page source..." : pageSources.length > 0 ? "Click a step to view its XML" : "No page source captured")}
                </div>
              </div>
            </div>
            ...
            {showFixPanel && (
              <div className="panel" style={{ marginTop: 12, border: "1px solid rgba(99,102,241,.4)", maxHeight: 520, overflow: "auto" }}>
                <div className="panel-header" style={{ background: "linear-gradient(135deg, rgba(99,102,241,.1), rgba(139,92,246,.1))", position: "sticky", top: 0, zIndex: 2 }}>
                  <div className="panel-title" style={{ color: "#a78bfa" }}>🤖 AI Fix Analysis</div>
                  <button className="btn-ghost btn-sm" onClick={() => { setShowFixPanel(false); setFixResult(null); }} style={{ fontSize: 10 }}>✕ Close</button>
                </div>
                {fixBusy && (
            ...
            <div className="steps-panel">
              ...
              <div className="steps-scroll">
                {stepDefs.map((s: any, i: number) => {
                  ...
                  return (
                    <div key={i} className={`step-item${isActive ? " running" : st === "failed" ? " fail" : ""}`} onClick={() => { if (res?.screenshot) setSelShot(res.screenshot); loadXmlForStep(i); }}>
...
function StepBuilder({ steps, setSteps }: { steps: any[]; setSteps: (s: any[]) => void }) {
  return (
...
function LibraryView({ project, tests, runs, modules, suites, onRefresh }: { project: Project; tests: TestDef[]; runs: Run[]; modules: ModuleDef[]; suites: SuiteDef[]; onRefresh: () => void }) {
  ...
  const aiGenerate = async () => {
    ...
    try { const res = await api.generateSteps(platform, aiPrompt, xml); setNewSteps(res.steps); setAiStatus(`Generated ${res.steps.length} steps`); toast(`AI generated ${res.steps.length} steps`, "success"); }
  ...
  const saveNew = async () => {
    ...
    setBusy(true);
    try { await api.createTest(project.id, { name: newName.trim(), steps: newSteps, suite_id: newSuiteId }); toast("Test saved", "success"); setNewName(""); setNewSteps([]); setShowCreate(false); onRefresh(); }
    catch (e: any) { toast(e.message, "error"); }
    finally { setBusy(false); }
  };

  const openEdit = (t: TestDef) => { setEditId(t.id); setEditName(t.name); setEditSteps([...t.steps]); setEditSuiteId(t.suite_id); setAiEditPrompt(""); setAiEditStatus(""); };
  const cancelEdit = () => { setEditId(null); };

  const saveEdit = async () => {
    if (!editId || !editName.trim()) return;
    setBusy(true);
    try { await api.updateTest(editId, { name: editName, steps: editSteps, suite_id: editSuiteId }); toast("Test updated", "success"); setEditId(null); onRefresh(); }
    catch (e: any) { toast(e.message, "error"); }
    finally { setBusy(false); }
  };
  ...
  const createMod = async () => {
    ...
    setBusy(true);
    try { await api.createModule(project.id, newModName.trim()); toast("Module created", "success"); setNewModName(""); onRefresh(); }
    catch (e: any) { toast(e.message, "error"); }
    finally { setBusy(false); }
  };
  ...
  return (
    <>
      <div className="section-head">
        <div><div className="section-title">Test Library</div><div className="section-sub">{tests.length} test cases · {modules.length} modules · {suites.length} suites</div></div>
        <div style={{ display: "flex", gap: 8 }}>
      ...
      <div className="panel" style={{ padding: 12, marginBottom: 12, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <input className="form-input" value={newModName} onChange={e => setNewModName(e.target.value)} placeholder="New module name" style={{ width: 160 }} />
        <button className="btn-ghost btn-sm" onClick={createMod} disabled={busy || !newModName.trim()}>+ Module</button>
      ...
      {editId && (
        <div className="panel" style={{ padding: 18, marginBottom: 16, border: "1px solid rgba(99,102,241,.4)" }}>
          ...
          <StepBuilder steps={editSteps} setSteps={setEditSteps} />
      ...
      <div className="panel">
        <table className="tc-table">
          <thead><tr><th>ID</th><th>Title</th><th>Module</th><th>Suite</th><th>Steps</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {tests.map(t => {
            ...
            {tests.length === 0 && <tr><td colSpan={7} style={{ color: "var(--muted)", padding: 18 }}>No tests yet. Click "+ New Test" to create one.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
...
function ReportsView({ project, runs, tests }: { project: Project; runs: Run[]; tests: TestDef[] }) {
  ...
  const exportModuleReport = async (mod: typeof moduleStats[0]) => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Module Report — ${mod.module}</title>
...
<h1>📁 Module Report — ${mod.module}</h1>
...
<div style="margin-top:32px;border-top:1px solid #1e2430;padding-top:16px;font-size:11px;color:#555">QA·OS Module Report · ${project.name} · ${mod.module} · ${new Date().toISOString()}</div>
</body></html>`;

    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([html], { type: "text/html" })); a.download = `module_report_${mod.module}.html`; a.click();
  };
  ...
  return (
    <>
      ...
      <div className="report-grid">
        ...
        <div className="panel">
          <div className="panel-header"><div className="panel-title">By Module</div></div>
          <div className="chart-bar-group">
            {moduleStats.map(ms => (
              <div key={ms.module} className="bar-row" style={{ cursor: "pointer" }} onClick={() => exportModuleReport(ms)}>
                ...
                <div className="bar-track"><div className="bar-fill" style={{ width: `${ms.rate}%`, background: ms.rate >= 90 ? "var(--accent)" : ms.rate >= 70 ? "var(--warn)" : "var(--danger)" }} /></div>
                <div className="bar-val">{ms.rate}%</div>
                <div style={{ fontSize: 10, color: "var(--accent2)", cursor: "pointer" }} title="Download module report">📋</div>
              </div>
            ))}
            {moduleStats.length === 0 && <div style={{ color: "var(--muted)", fontSize: 12 }}>Create tests to see module breakdown</div>}
          </div>
